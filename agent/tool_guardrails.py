...
def classify_tool_failure(tool_name: str, result: str | None) -> tuple[bool, str]:
    ...
    Delegates to ``agent.display._detect_tool_failure`` so the guardrail
    never disagrees with the CLI's user-visible ``[error]`` tag. Production
    callers always pass an explicit ``failed=`` derived from that helper;
    this function exists so standalone callers (tests, tooling) still get
    consistent behavior — including correct handling of multi-result tools
    that serialize ``"error": null`` on success (web_extract).
    """
    # Lazy import avoids a hard cycle at module load (display is heavy and
    # must not import tool_guardrails).
